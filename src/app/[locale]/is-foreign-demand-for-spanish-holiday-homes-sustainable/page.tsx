...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return getPageMetadata('blog-demand', locale as Locale);
}

export default async function BlogDemandPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = getMessages(locale as Locale);
  const a = t.blog_demand;
  ...
  const articleJsonLd = getArticleJsonLd({
    ...
    image: `${IMG}/2016/12/pisina-118-ejemplo-grande.jpg`,
  });
  ...
  const related = [
    { key: 'blog-foreigners' as const, label: t.blog_foreigners.label, title: t.blog_foreigners.title },
    { key: 'blog-barcelona' as const, label: t.blog_barcelona.label, title: t.blog_barcelona.title },
    { key: 'blog-brexit' as const, label: t.blog_brexit.label, title: t.blog_brexit.title },
  ];
    ...
    <>
      ...
      <section className="relative min-h-[60vh] flex items-end overflow-hidden bg-navy-dark">
        <div className="absolute inset-0">
          <Image
            src={`${IMG}/2016/12/pisina-118-ejemplo-grande.jpg`}
            alt="Spanish holiday home with pool"
            fill
