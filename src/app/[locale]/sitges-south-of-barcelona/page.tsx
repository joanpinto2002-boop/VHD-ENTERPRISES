...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return getPageMetadata('blog-sitges', locale as Locale);
}

export default async function BlogSitgesPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = getMessages(locale as Locale);
  const a = t.blog_sitges;
  ...
  const articleJsonLd = getArticleJsonLd({
    ...
    image: `${IMG}/2016/12/piscina-vistas-doc.jpg`,
  });
    ...
    <>
      ...
      <section className="relative min-h-[60vh] flex items-end overflow-hidden bg-navy-dark">
        <div className="absolute inset-0">
          <Image
            src={`${IMG}/2016/12/piscina-vistas-doc.jpg`}
            alt="Mediterranean villa with pool near Sitges"
            fill
