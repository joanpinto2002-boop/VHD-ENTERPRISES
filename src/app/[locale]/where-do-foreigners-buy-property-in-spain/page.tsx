...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return getPageMetadata('blog-foreigners', locale as Locale);
}

export default async function BlogForeignersPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = getMessages(locale as Locale);
  const a = t.blog_foreigners;

  const articleJsonLd = getArticleJsonLd({
    page: 'blog-foreigners',
    locale: locale as Locale,
    title: a.title,
    description: a.intro,
    datePublished: '2019-12-01',
    dateModified: '2026-03-28',
    image: `${IMG}/2016/12/096.jpg`,
  });
    ...
    <>
      ...
      <section className="relative min-h-[60vh] flex items-end overflow-hidden bg-navy-dark">
        <div className="absolute inset-0">
          <Image
            src={`${IMG}/2016/12/096.jpg`}
            alt="Spanish property — where foreigners buy"
            fill
