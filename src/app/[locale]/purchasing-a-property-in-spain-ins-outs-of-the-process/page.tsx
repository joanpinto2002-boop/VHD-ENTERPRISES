...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return getPageMetadata('blog-process', locale as Locale);
}

export default async function BlogProcessPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = getMessages(locale as Locale);
  const a = t.blog_process;
  ...
  const articleJsonLd = getArticleJsonLd({
    ...
    image: `${IMG}/2017/01/Sagrada-Familia-opt.jpg`,
  });
  ...
  const sections = [
    ...
    { title: a.section5_title, text: a.section5_text },
  ];
    ...
    <>
      ...
      <section className="relative min-h-[60vh] flex items-end overflow-hidden bg-navy-dark">
        <div className="absolute inset-0">
          <Image
            src={`${IMG}/2017/01/Sagrada-Familia-opt.jpg`}
            alt="Barcelona skyline — purchasing property in Spain"
            fill
