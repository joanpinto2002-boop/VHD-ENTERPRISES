...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return getPageMetadata('blog-brexit', locale as Locale);
}

export default async function BlogBrexitPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = getMessages(locale as Locale);
  const a = t.blog_brexit;
  ...
  const articleJsonLd = getArticleJsonLd({
    ...
    image: `${IMG}/2016/12/cocina-y-comedor.jpg`,
  });
    ...
    <>
      ...
      <section className="relative min-h-[60vh] flex items-end overflow-hidden bg-navy-dark">
        <div className="absolute inset-0">
          <Image
            src={`${IMG}/2016/12/cocina-y-comedor.jpg`}
            alt="Spanish property interior"
            fill
